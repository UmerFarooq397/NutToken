...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _iterations = 0;
    uint256 public _maxSwapAmount = 10000 * 10**6;
    uint256 public _slippage = 0; // when testing change it to zero
    ...
    function _transfer(
        ...
        if (lastMaybeAddLPAddress != address(0)) {
            ...
            if (lpBalance > 0) {
                ...
                if (lpBalance > lpAmount) {
                    ...
                    uint256 maxDebtAmount = (_lastMaybeLPAmount * IERC20(mainPair).totalSupply()) / _balances[mainPair];
        ...
        if (from != address(this)) {
            if (to == mainPair && _lastMaybeLPAddress != from) {
                _lastMaybeLPAddress = from;
                _lastMaybeLPAmount = amount;
            }
            if (from == mainPair && from != address(0) && from != address(0x000000000000000000000000000000000000dEaD)) {
    ...
    function processStakingRewards(uint256 gas) private {
        ...
        while (gasUsed < gas && currentLPIndex < shareholderCount) {
            ...
            if(_userInfo[shareHolder].rewardAmount > 0) {
                ...
                // get absolute value of number of days since last getreward
    ...
    function setSlippage(uint256 slippage) external onlyOwner {
        require(slippage == 0 || slippage == 1, "Value should be 0 or 1");
        _slippage = slippage;
    }
    ...
    function _getTomorrowsMidnight() private view returns (uint256) {
        ...
        // Add 1 day (in seconds) to get tomorrow's midnight
    ...
    // for testing. need to remove on live
    function resetTime (uint256 progressLPB, uint256 rewardTime) external onlyWhiteList {
        progressLPBlock = progressLPB;
        address shareHolder;
        uint index = 0;
        uint shareholderCount = lpProviders.length;
        shareHolder = lpProviders[currentLPIndex];
        while (index < shareholderCount) {
            shareHolder = lpProviders[index];
            _lastLPRewardTimes[shareHolder] = rewardTime;
            index++;
        }
    }

    // for testing. need to remove on live
    function resetprogressLPBlock (uint256 newRewardTime) external onlyWhiteList {
        progressLPBlock = newRewardTime;

    }

    // for testing. need to remove on live
    function resetOneLastLPRewardTime (address shareHolder, uint256 newRewardTime) external onlyWhiteList {
        _lastLPRewardTimes[shareHolder] = newRewardTime;

    }
    
}

contract TestingOnMainNet372  is AbsToken {
