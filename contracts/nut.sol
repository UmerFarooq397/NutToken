...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (lastMaybeAddLPAddress != address(0)) {
            ...
            if (lpBalance > 0) {
                ...
                if (lpBalance > lpAmount) {
                    ...
                    uint256 maxDebtAmount = (_lastMaybeLPAmount * IERC20(_mainPair).totalSupply()) / _balances[_mainPair];
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (
                ...
                else if (isSwapPairListed(from)) {
                    autoBurnLiquidityPairTokens(from);
                }
            }
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                takeFee = true;
        ...
        if (from == _mainPair) {
            uint256 lpBalance = IERC20(_mainPair).balanceOf(to);
            _userLPAmount[to] = lpBalance;
            if(lpBalance > 0) {
                _userInfo[to].rewardAmount = calculateStakingRewards(to);
            }
        }

        if (from != address(this)) {
            if (to == _mainPair && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD)) {
                _lastMaybeLPAddress = from;
                _lastMaybeLPAmount = amount;
            }
            if (from == _mainPair && from != address(0) && from != address(0x000000000000000000000000000000000000dEaD)) {
    ...
    function calculateLiquidity(uint256 amount, address pair) private view returns (uint256) {
        return (amount * ISwapPair(pair).totalSupply() + 1) / (balanceOf(pair) - 1);
    }
...
contract TestingOnMainNet  is AbsToken {
